...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const [editingAdultId, setEditingAdultId] = useState<string | null>(null);
  const [editAdultTitle, setEditAdultTitle] = useState('');
  const [editAdultThumb, setEditAdultThumb] = useState('');
  const [editAdultLink, setEditAdultLink] = useState('');
  const [editAdultCategory, setEditAdultCategory] = useState('');
  const [adultCategories, setAdultCategories] = useState<string[]>(['General', 'Videos', 'Series', 'Photos', 'Trending']);
  const [newCatInput, setNewCatInput] = useState('');
  const [adultDuration, setAdultDuration] = useState('');
  const [adultIsPremium, setAdultIsPremium] = useState(false);
  const [adultPremiumPrice, setAdultPremiumPrice] = useState('5');
  const [editAdultIsPremium, setEditAdultIsPremium] = useState(false);
  const [editAdultPremiumPrice, setEditAdultPremiumPrice] = useState('5');
  const [editAdultDuration, setEditAdultDuration] = useState('');
  ...
  // ✅ 18+ Settings
  const [adultBannerUrl, setAdultBannerUrl] = useState('');
  const [adultSectionHidden, setAdultSectionHidden] = useState(false);
  const [adultTutorialBanner, setAdultTutorialBanner] = useState('');
  const [adultTutorialLink, setAdultTutorialLink] = useState('');
  // ✅ Task System
  ...
  const applySettingsData = (data: any) => {
    ...
    setAdultBannerUrl(data.adultBannerUrl || '');
    // adultDefaultPrice handled in settings save
    setAdultSectionHidden(data.adultSectionHidden ?? false);
    setAdultTutorialBanner(data.adultTutorialBanner || '');
    setAdultTutorialLink(data.adultTutorialLink || '');
  };
  ...
  const fetchAdult = async () => {
    try {
      ...
      const cats = Array.from(new Set(items.map((i: any) => i.category || 'General').filter(Boolean)));
      if (cats.length > 0) {
        setAdultCategories(prev => Array.from(new Set([...prev, ...cats])));
      }
  ...
  const handleAddAdult = async () => {
    ...
    try {
      await addDoc(collection(db, 'adultContent'), {
        ...
        duration: adultDuration.trim() || '',
        isPremium: adultIsPremium,
        premiumPrice: adultIsPremium ? (parseFloat(adultPremiumPrice) || 5) : 0,
        views: 0,
      ...
      setAdultCategory('');
      setAdultDuration('');
      setAdultIsPremium(false);
      setAdultPremiumPrice('5');
  ...
  const handleEditAdult = (item: AdultBannerItem) => {
    setEditingAdultId(item.id);
    setEditAdultTitle(item.title);
    setEditAdultThumb(item.thumbnail);
    setEditAdultLink(item.channelLink);
    setEditAdultCategory(item.category || '');
    setEditAdultDuration((item as any).duration || '');
    setEditAdultIsPremium(item.isPremium || false);
    setEditAdultPremiumPrice(String(item.premiumPrice || 5));
  };

  const handleUpdateAdult = async (id: string) => {
    if (!editAdultTitle.trim() || !editAdultThumb.trim() || !editAdultLink.trim()) {
      alert('Title, Thumbnail, Link সব দিতে হবে!'); return;
    }
    try {
      await updateDoc(doc(db, 'adultContent', id), {
        title: editAdultTitle.trim(),
        thumbnail: editAdultThumb.trim(),
        channelLink: editAdultLink.trim(),
        category: editAdultCategory.trim() || 'General',
        duration: editAdultDuration.trim() || '',
        isPremium: editAdultIsPremium,
        premiumPrice: editAdultIsPremium ? (parseFloat(editAdultPremiumPrice) || 5) : 0,
      });
      setEditingAdultId(null);
      showSuccess('✅ Updated!');
      fetchAdult();
    } catch(e) { console.warn(e); alert('Error! Check Firestore rules.'); }
  };
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const settingsData = {
        ...
        adultBannerUrl: adultBannerUrl || '',
        adultSectionHidden: adultSectionHidden,
        adultTutorialBanner: adultTutorialBanner || '',
        adultTutorialLink: adultTutorialLink || '',
      };
            ...
            <div className="space-y-6">
              ...
              <div className="bg-[#18181b] rounded-2xl p-4 space-y-3 border border-white/5">
                ...
                <div>
                  ...
                  <input type="text" value={adultTitle} onChange={e => setAdultTitle(e.target.value)}
                    placeholder="Content এর নাম..." className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-pink-500" />
                </div>

                <div>
                  <label className="text-xs text-zinc-400 mb-1 block">Thumbnail URL * (16:9 image)</label>
                  <input type="text" value={adultThumb} onChange={e => setAdultThumb(e.target.value)}
                    placeholder="https://... (image link)" className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-pink-500" />
                  {adultThumb && <img src={adultThumb} className="mt-2 w-full aspect-video object-cover rounded-xl border border-white/10 max-w-[240px]" />}
                </div>

                <div>
                  <label className="text-xs text-zinc-400 mb-1 block">Watch Link *</label>
                  <input type="text" value={adultLink} onChange={e => setAdultLink(e.target.value)}
                    placeholder="https://t.me/..." className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-pink-500" />
                </div>

                <div>
                  <label className="text-xs text-zinc-400 mb-1 block">Duration (যেমন: 3:45, 25:20)</label>
                  <input type="text" value={adultDuration} onChange={e => setAdultDuration(e.target.value)}
                    placeholder="3:45" className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-pink-500" />
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {adultCategories.map(cat => (
                      <button key={cat} onClick={() => setAdultCategory(cat)}
                        className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${adultCategory === cat ? 'bg-pink-500 text-white' : 'bg-[#242427] text-zinc-400 border border-white/10'}`}>
                        {cat}
                      </button>
                    ))}
                  </div>
                  <div className="flex gap-2">
                    <input type="text" value={newCatInput} onChange={e => setNewCatInput(e.target.value)}
                      placeholder="নতুন category লেখো..." className="flex-1 bg-[#242427] border border-white/10 rounded-xl px-3 py-2 text-white text-xs outline-none" />
                    <button onClick={() => { if (newCatInput.trim()) { setAdultCategories(p => [...p, newCatInput.trim()]); setAdultCategory(newCatInput.trim()); setNewCatInput(''); }}}
                      className="px-3 py-2 bg-pink-500/20 text-pink-400 rounded-xl text-xs font-bold border border-pink-500/20">+ Add</button>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-3 bg-[#242427] rounded-xl">
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">Taka দিয়ে unlock হবে</p>
                  </div>
                  <button onClick={() => setAdultIsPremium(p => !p)}
                    className={`relative w-12 h-6 rounded-full transition-all ${adultIsPremium ? 'bg-amber-500' : 'bg-zinc-600'}`}>
                    <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${adultIsPremium ? 'left-6' : 'left-0.5'}`} />
                  </button>
                </div>
                {adultIsPremium && (
                  <div>
                    <label className="text-xs text-zinc-400 mb-1 block">Price (৳ Taka)</label>
                    <input type="number" value={adultPremiumPrice} onChange={e => setAdultPremiumPrice(e.target.value)}
                      placeholder="5" className="w-full bg-[#242427] border border-amber-500/30 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-amber-500" />
                  </div>
                )}

                <button onClick={handleAddAdult} disabled={adultLoading}
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold text-white">Content ({adultItems.filter(i => !adultSearchQuery || i.title.toLowerCase().includes(adultSearchQuery.toLowerCase())).length}/{adultItems.length})</h3>
                </div>
                {/* Search */}
                <div className="relative mb-3">
                  <input type="text" value={adultSearchQuery} onChange={e => setAdultSearchQuery(e.target.value)}
                  ...
                  {adultSearchQuery && (
                    <button onClick={() => setAdultSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-white text-xs">✕</button>
                  )}
                </div>
                {adultItems.length === 0 ? (
                  <p className="text-zinc-500 text-sm text-center py-8">কোনো content নেই। উপরে form দিয়ে যোগ করো।</p>
                ) : (
                  <div className="space-y-3">
                    {adultItems.filter(i => !adultSearchQuery || i.title.toLowerCase().includes(adultSearchQuery.toLowerCase())).map(item => (
                      <div key={item.id} className="bg-[#18181b] rounded-xl p-3 border border-white/5">
                        {editingAdultId === item.id ? (
                          /* ── Edit Mode ── */
                          <div className="space-y-2">
                            <input type="text" value={editAdultTitle} onChange={e => setEditAdultTitle(e.target.value)}
                              placeholder="Title" className="w-full bg-[#242427] border border-pink-500/30 rounded-xl px-3 py-2 text-white text-sm outline-none" />
                            <input type="text" value={editAdultThumb} onChange={e => setEditAdultThumb(e.target.value)}
                              placeholder="Thumbnail URL" className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2 text-white text-sm outline-none" />
                            {editAdultThumb && <img src={editAdultThumb} className="w-full max-w-[180px] aspect-video object-cover rounded-lg border border-white/10" />}
                            <input type="text" value={editAdultLink} onChange={e => setEditAdultLink(e.target.value)}
                              placeholder="Watch Link" className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2 text-white text-sm outline-none" />
                            <input type="text" value={editAdultDuration} onChange={e => setEditAdultDuration(e.target.value)}
                              placeholder="Duration (3:45)" className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2 text-white text-sm outline-none" />
                            <div className="flex flex-wrap gap-1.5">
                              {adultCategories.map(cat => (
                                <button key={cat} onClick={() => setEditAdultCategory(cat)}
                                  className={`px-2.5 py-1 rounded-lg text-xs font-bold ${editAdultCategory === cat ? 'bg-pink-500 text-white' : 'bg-[#111] text-zinc-400 border border-white/10'}`}>
                                  {cat}
                                </button>
                              ))}
                            </div>
                            <div className="flex items-center justify-between p-2 bg-[#111] rounded-xl mt-1">
                              <span className="text-xs font-bold text-white">💳 Premium</span>
                              <button onClick={() => setEditAdultIsPremium(p => !p)}
                                className={`relative w-10 h-5 rounded-full transition-all ${editAdultIsPremium ? 'bg-amber-500' : 'bg-zinc-700'}`}>
                                <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all ${editAdultIsPremium ? 'left-5' : 'left-0.5'}`} />
                              </button>
                            </div>
                            {editAdultIsPremium && (
                              <input type="number" value={editAdultPremiumPrice} onChange={e => setEditAdultPremiumPrice(e.target.value)}
                                placeholder="৳ Price" className="w-full bg-[#111] border border-amber-500/30 rounded-xl px-3 py-2 text-white text-sm outline-none" />
                            )}
                            <div className="flex gap-2 mt-1">
                              <button onClick={() => handleUpdateAdult(item.id)}
                                className="flex-1 py-2 rounded-xl bg-green-500 text-black text-sm font-black">✅ Save</button>
                              <button onClick={() => setEditingAdultId(null)}
                                className="flex-1 py-2 rounded-xl bg-white/10 text-white text-sm font-bold">Cancel</button>
                            </div>
                          </div>
                        ) : (
                          /* ── View Mode ── */
                          <div className="flex gap-3">
                            <div className="relative w-24 shrink-0">
                              <img src={item.thumbnail} className="w-full aspect-video object-cover rounded-lg" />
                              {item.category && (
                                <div className="absolute bottom-1 left-1 bg-black/70 rounded px-1.5 py-0.5">
                                  <span className="text-[9px] text-white font-bold">{item.category}</span>
                                </div>
                              )}
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              {item.isPremium && <span className="text-[10px] font-bold text-amber-400">💳 ৳{item.premiumPrice}</span>}
                              {(item as any).duration && <span className="text-[10px] text-zinc-500">⏱ {(item as any).duration}</span>}
                              <span className="text-[10px] text-zinc-600">👁 {(item as any).views || 0} views</span>
                            </div>
                            <div className="flex gap-1.5 mt-2 flex-wrap">
                              <button onClick={() => handleEditAdult(item)}
                                className="text-xs px-2.5 py-1 rounded-lg bg-blue-500/20 text-blue-400 font-bold">✏️ Edit</button>
                              <button onClick={() => handleToggleAdult(item.id, item.isActive)}
                              ...
                              <button onClick={() => handleDeleteAdult(item.id)}
                                className="text-xs px-2.5 py-1 rounded-lg bg-red-500/20 text-red-400 font-bold">🗑 Delete</button>
                            </div>
                            </div>
                          </div>
                        )}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-[#18181b] border border-pink-500/20 rounded-[20px] p-5 space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={adultBannerUrl}
                    onChange={e => setAdultBannerUrl(e.target.value)}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={adultTutorialBanner}
                    onChange={e => setAdultTutorialBanner(e.target.value)}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={adultTutorialLink}
                    onChange={e => setAdultTutorialLink(e.target.value)}
                ...
                <div className="flex items-center justify-between p-3 bg-[#242427] rounded-xl">
                  ...
                  <button
                    onClick={() => setAdultSectionHidden(prev => !prev)}
                    className={`relative w-12 h-6 rounded-full transition-all ${adultSectionHidden ? 'bg-red-500' : 'bg-zinc-600'}`}
                  >
                    <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${adultSectionHidden ? 'left-6' : 'left-0.5'}`} />
                  </button>
                </div>
              </div>
