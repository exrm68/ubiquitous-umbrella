...
const CinematicLockScreen: React.FC<LockScreenProps> = ({
  movie, total, watched, actionType, skipped, adLoading, savedProgress, tutorialLink, cooldown = 0, onClose, onWatchAd
}) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 pb-10 text-center">
        ...
        {/* CTA Button */}
        <motion.button whileTap={{ scale: cooldown > 0 ? 1 : 0.95 }} onClick={onWatchAd} disabled={adLoading || cooldown > 0}
          style={{
          ...
          {adLoading ? (
            ...
            <><span className="w-7 h-7 rounded-full border-2 border-black/40 flex items-center justify-center font-black text-sm">{cooldown}</span>অপেক্ষা করুন...</>
          ) : skipped ? (
...
const EpisodeLockOverlay: React.FC<EpisodeLockOverlayProps> = ({
  movie, episode, total, watched, skipped, adLoading, savedProgress, tutorialLink, cooldown = 0, onClose, onWatchAd
}) => {
  const remaining = total - watched;
    ...
    <>
      ...
      <motion.div
        ...
        <div className="flex flex-col items-center px-6 pt-3 pb-10 text-center">
          ...
          <motion.button whileTap={{ scale: cooldown > 0 ? 1 : 0.95 }} onClick={onWatchAd} disabled={adLoading || cooldown > 0}
            style={{
              background: cooldown > 0 ? 'linear-gradient(135deg,#78716c,#57534e)' : skipped ? 'linear-gradient(135deg,#ef4444,#dc2626)' : 'linear-gradient(135deg,#f59e0b,#d97706)',
            ...
            {adLoading ? (
              ...
              <><span className="w-6 h-6 rounded-full border-2 border-black/40 flex items-center justify-center font-black text-xs">{cooldown}</span>অপেক্ষা করুন...</>
            ) : skipped ? (
...
const MovieDetails: React.FC<MovieDetailsProps> = ({ movie, onClose, botUsername, channelLink, appSettings }) => {
  ...
  const [coinToast, setCoinToast] = useState<string>(''); // '+5 🪙' etc
  ...
  return (
    <>
            ...
            <motion.div initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
              ...
              {!isSeries && (
                <div className="flex flex-col gap-3 w-full mb-7">
                  {/* Watch */}
                  <button onClick={() => cooldownRemaining > 0 ? null : handleWatch(movie.telegramCode)}
                    ...
                    style={{
                      background: cooldownRemaining > 0 ? '#3f3f46' : watchUnlocked ? 'linear-gradient(135deg,#22c55e,#15803d)' : '#FFD700',
                      boxShadow: cooldownRemaining > 0 ? 'none' : watchUnlocked ? '0 0 20px rgba(34,197,94,0.25)' : '0 0 20px rgba(255,215,0,0.2)',
                      color: cooldownRemaining > 0 ? '#a1a1aa' : '#000',
                      cursor: cooldownRemaining > 0 ? 'not-allowed' : 'pointer',
                    }}>
                    {cooldownRemaining > 0 ? (
                      <><span className="w-6 h-6 rounded-full border-2 border-zinc-500 flex items-center justify-center font-black text-xs text-zinc-400">{cooldownRemaining}</span><span className="text-zinc-400">অপেক্ষা করুন...</span></>
                    ) : watchUnlocked ? (
                  ...
                  {/* Download */}
                  <button onClick={() => cooldownRemaining > 0 ? null : handleDownload(movie.downloadCode, movie.downloadLink, movie.telegramCode)}
                    disabled={cooldownRemaining > 0}
                    ...
                    {cooldownRemaining > 0 ? (
                      <><span className="w-6 h-6 rounded-full border-2 border-zinc-600 flex items-center justify-center font-black text-xs text-zinc-500">{cooldownRemaining}</span><span className="text-zinc-500">অপেক্ষা করুন...</span></>
                    ) : downloadUnlocked ? (
      ...
      {/* ── Cinematic Lock Screen ── */}
      <AnimatePresence>
        {/* cooldown এখন lock screen button এ দেখায় — floating toast বাদ */}
        ...
        <AnimatePresence>
          {coinToast && (
            <motion.div
              ...
              className="fixed bottom-32 left-1/2 -translate-x-1/2 z-[400] pointer-events-none"
            >
              <div className="bg-amber-400 text-black px-5 py-2.5 rounded-full font-black text-base shadow-[0_4px_20px_rgba(251,191,36,0.5)] flex items-center gap-2">
                <span>{coinToast}</span>
                <span className="text-xs font-bold opacity-70">earned!</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
